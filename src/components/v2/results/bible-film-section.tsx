...
export function BibleFilmSection({ bible, loading, provider, onGenerate }: BibleFilmSectionProps) {
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-4">
        <h3 className="text-sm font-semibold text-bw-heading uppercase tracking-wide">Bible du film</h3>
        {!bible && !loading && (
          <button
        ...
        <div className="grid gap-4">
                ...
                {bible.characters.map((c, i) => (
                  ...
                  >
                    <div className="flex items-center gap-2 mb-1">
                      <span className="text-sm font-semibold text-bw-heading">{c.name}</span>
                      <span className="text-xs rounded-md bg-card px-1.5 py-0.5 text-bw-muted border border-[var(--color-bw-border)]">
                        {c.role}
                      </span>
                    </div>
                    <p className="text-xs text-bw-heading mb-1">{c.description}</p>
                ...
                <div key={act} className="flex gap-3">
                  ...
                  <div>
                    <p className="text-xs font-bold text-bw-muted uppercase mb-0.5">
                      {i === 0 ? "Exposition" : i === 1 ? "Confrontation" : "Résolution"}
                    </p>
                    <p className="text-sm text-bw-heading">{bible.structure[act]}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          {provider && (
            <p className="text-xs text-bw-muted text-right">
              Généré par {provider === "fallback" ? "algorithme" : provider}
            </p>
          )}
        </div>
      )}
