    ...
    <>
      ...
      <div className="glass-card p-4 space-y-2">
        ...
        <p className="text-xs text-bw-muted leading-relaxed">
          Chaque élève dispose de crédits pour composer son budget de film. Il choisit un niveau (basique, standard ou
          premium) pour chaque poste de production : acteurs, décors, effets, musique, lumière, caméra.
        </p>
      </div>
      {/* Submission counter */}
      <div className="bg-bw-teal/10 rounded-xl p-4 border border-bw-teal/20">
        <div className="flex items-center justify-between">
          ...
          <span className="text-xl font-bold text-bw-teal tabular-nums">
            {budgetSubmitted}/{activeStudentCount}
          </span>
        </div>
        <div className="mt-2 h-2 bg-bw-bg rounded-full overflow-hidden">
          <div
            ...
            style={{
              width: `${activeStudentCount > 0 ? Math.round((budgetSubmitted / activeStudentCount) * 100) : 0}%`,
            }}
          />
        </div>
      </div>
