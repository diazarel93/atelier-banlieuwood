    ...
    <>
      ...
      {budgets.length > 0 && (
        <div className="space-y-2">
          {budgets
            .filter(
              (b) => !cardSearch || (b.students?.display_name || "").toLowerCase().includes(cardSearch.toLowerCase()),
            )
            .map((b) => (
                    ...
                    return (
                      <div key={cat.key} className="flex-1 text-center">
                        <div
                          ...
                          style={{
                            backgroundColor: cost > 0 ? cat.color : "rgba(0,0,0,0.05)",
                            opacity: cost > 0 ? 0.7 : 1,
                          }}
                        />
                        <span className="text-xs block" style={{ color: cat.color }}>
                          {cat.label}
                        </span>
