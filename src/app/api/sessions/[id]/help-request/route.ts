...
export const POST = withErrorHandler(async function POST(
  ...
  if (!studentId || !step || !helpType) {
    return NextResponse.json({ error: "studentId, step et helpType requis" }, { status: 400 });
  }
  ...
  const { data: session } = await admin.from("sessions").select("help_enabled").eq("id", sessionId).single();

  if (!session?.help_enabled) {
    return NextResponse.json({ error: "L'aide n'est pas encore activée par l'intervenant" }, { status: 403 });
  }
  ...
  if ((count || 0) >= MAX_HELP_PER_STEP) {
    return NextResponse.json({ error: `Maximum ${MAX_HELP_PER_STEP} aides par étape` }, { status: 400 });
  }

  // Log help request
  await admin.from("module10_help_requests").insert({
