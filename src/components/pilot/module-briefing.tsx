...
const MODULE_ACTIVITIES: Record<string, ActivityInfo> = {
  ...
  m11c: { type: "debate", emoji: "🦸", label: "Héros & Anti-Héros", detail: "Sacrifice, pardon, trahison. Qu'est-ce qui fait un vrai héros ? Et un bon méchant ? Argumentation et confrontation.", tags: ["Héros", "Morale", "Débat"] },
  m11d: { type: "debate", emoji: "🎥", label: "Les Coulisses", detail: "Budget, cascades, IA, montage. Les secrets de fabrication du cinéma. L'IA et l'avenir du cinéma en débat final.", tags: ["Production", "IA", "Débat"] },
};
...
function getQuestionLayout(type: string): "grid" | "featured" | "list" {
  switch (type) {
    case "quiz": return "grid";
    case "image":
    case "notebook": return "featured";
    default: return "list";
  }
}
...
export function ModuleBriefing({
  ...
  useEffect(() => {
    ...
    fetch(`/api/sessions/${sessionId}/situations-preview?module=${module.dbModule}&seance=${module.dbSeance}`)
      ...
      .then((d) => { if (d?.situations) setQuestions(d.situations); })
      .catch(() => {})
        ...
        >
          ...
          {!cover && (
            <div
              ...
              style={{ background: `linear-gradient(145deg, ${module.color}30 0%, ${module.color}12 40%, #08090E 100%)` }}
            />
          )}
          ...
          {/* Scanlines */}
          <div className="absolute inset-0 pointer-events-none opacity-[0.04]" style={{ backgroundImage: "repeating-linear-gradient(0deg, transparent, transparent 2px, rgba(0,0,0,0.4) 2px, rgba(0,0,0,0.4) 4px)" }} />
          ...
          <div className="relative px-6 pt-5 pb-5 space-y-4 min-h-[200px] flex flex-col justify-end">
            ...
            <div className="flex items-center gap-4">
              <motion.div
                ...
                style={{ background: `linear-gradient(135deg, ${module.color}, ${module.color}AA)`, boxShadow: `0 8px 24px ${module.color}40` }}
              >
        ...
        <div className="space-y-4 mt-5">
          ...
          {activity && (
            ...
            >
                ...
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-1.5 mt-0.5">
                    {activity.tags.map((tag) => (
                      <span key={tag} className="text-xs font-medium px-2 py-0.5 rounded-full bg-black/[0.04] text-white/50">
                        {tag}
                      </span>
                    ))}
                  </div>
              ...
              {/* Description */}
              <div className="px-5 py-3" style={{ background: `linear-gradient(180deg, ${module.color}08, transparent)` }}>
              ...
              {actType === "game" && (
                <div className="px-5 pb-3 flex items-center gap-2">
                  {["🎭", "📍", "🔊", "⏰", "💬"].map((token, i) => (
                    <div key={i} className="w-8 h-8 rounded-lg bg-black/[0.03] border border-black/[0.06] flex items-center justify-center text-sm">
                      {token}
                    </div>
                  ))}
          ...
          >
            <div className="px-4 py-3 flex items-center gap-2" style={{ background: `linear-gradient(135deg, ${module.color}12, ${module.color}05)` }}>
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={module.color} strokeWidth="2" strokeLinecap="round">
                ...
                <>
                  ...
                  {questionLayout === "list" && (
                    <div className="pt-2 relative">
                      {/* Vertical line */}
                      <div className="absolute left-[14px] top-4 bottom-2 w-px" style={{ background: `linear-gradient(180deg, ${module.color}20, transparent)` }} />
