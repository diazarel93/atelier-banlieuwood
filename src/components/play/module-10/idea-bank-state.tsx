...
export function IdeaBankState({ module10, sessionId, studentId, onDone }: IdeaBankStateProps) {
      ...
      {ideas.length > 0 && (
        <div className="w-full space-y-2 max-h-60 overflow-y-auto">
          {ideas.map((idea) => (
            <div
              key={idea.id}
              className="flex items-start gap-2 p-2 rounded-lg bg-bw-elevated border border-white/[0.06]"
            >
      ...
      {!module10.submitted && (
        <>
          <textarea
            ...
            placeholder="Partage ton « Et si... » préféré..."
            rows={2}
            maxLength={300}
