...
export function PilotTopBar({
  ...
  return (
    <header
      className="glass border-b border-black/[0.06] px-3 py-2 flex-shrink-0 z-20"
    ...
    >
      <div className="flex items-center gap-2">
        ...
        <div className="flex items-center gap-2 min-w-0">
          ...
          <span className="text-sm font-medium text-bw-text truncate max-w-[200px] hidden md:inline">
            {sessionTitle}
          </span>
        </div>
          ...
          {activeModuleLabel && !isDone && (
            <button
              ...
              className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl border text-xs cursor-pointer transition-colors duration-200 ${
                isPaused
                  ? "bg-bw-amber/10 border-bw-amber/30 text-bw-amber"
                  : "bg-bw-elevated border-black/[0.04] text-bw-muted hover:text-bw-heading"
              }`}
            >
              <svg
                width="12"
        ...
        <div className="flex items-center gap-2">
          ...
          {hasTimer && (
            <div
              className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl bg-bw-primary/15 border border-bw-primary/30"
              style={{ boxShadow: "0 0 12px rgba(255,107,53,0.15)" }}
            >
              <CountdownTimer endsAt={timerEndsAt!} size="sm" onExpired={onTimerExpired} />
              <button
                onClick={onClearTimer}
                className="text-xs text-bw-primary hover:text-bw-heading cursor-pointer ml-0.5"
              >
                ✕
              </button>
            </div>
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl border text-xs cursor-pointer transition-colors duration-200 ${
              showStudents
                ...
                : "bg-bw-elevated border-black/[0.04] text-bw-text hover:text-bw-heading"
            }`}
          >
            <motion.div
                  ...
                  >
                    ...
                    {onShortcuts && (
                      ...
                      >
                        ...
                        <kbd className="ml-auto w-4 h-4 rounded bg-black/[0.05] text-[9px] font-mono flex items-center justify-center text-bw-muted">
                          ?
                        </kbd>
                      </button>
                    )}
