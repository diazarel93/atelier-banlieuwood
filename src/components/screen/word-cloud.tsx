...
const STOP_WORDS = new Set([
  ...
  "me", "te", "se", "en", "y", "qui", "que", "quoi", "dont", "ou",
  "et", "ou", "mais", "donc", "car", "ni", "ne", "pas", "plus",
  "dans", "sur", "sous", "avec", "sans", "pour", "par", "vers", "chez",
  "est", "sont", "a", "ont", "fait", "été", "être", "avoir",
  "comme", "aussi", "très", "bien", "tout", "tous", "toute", "toutes",
  "peut", "faire", "dit", "va", "vont", "cest", "c'est", "qu'il",
  "qu'elle", "qu'on", "il y", "ya",
]);
...
export function WordCloud({ texts, maxWords = 30, accentColor = "#FF6B35" }: WordCloudProps) {
  const words = useMemo(() => {
    ...
    return sorted.map(([word, count]): WordEntry => ({
      word,
      count,
