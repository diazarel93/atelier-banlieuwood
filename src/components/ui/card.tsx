  ...
  "flex flex-col gap-5 rounded-xl text-bw-text transition-all duration-200 ease-[cubic-bezier(0.4,0,0.2,1)]",
  {
    variants: {
      variant: {
        ...
        ghost: "bg-transparent border border-transparent hover:bg-bw-surface/50 hover:border-bw-border-subtle",
      },
    },
    defaultVariants: {
      variant: "default",
...
function CardDescription({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div data-slot="card-description" className={cn("text-bw-muted text-sm leading-relaxed", className)} {...props} />
  );
}
...
function CardFooter({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn("flex items-center pt-4 border-t border-bw-border-subtle [.border-t]:pt-5", className)}
      {...props}
    />
  );
}
