...
export function PitchAssemblyState({ module10, sessionId, studentId, onDone }: PitchAssemblyStateProps) {
    ...
    async (text: string) => {
      ...
      try {
        await fetch(`/api/sessions/${sessionId}/pitch`, {
          ...
          body: JSON.stringify({
            studentId,
            objectif: module10.objectif || "",
            obstacle: module10.obstacle || "",
            pitchText: text.trim(),
          }),
        });
  ...
  const OBJECTIF_LABELS: Record<string, string> = {
    sauver: "Sauver quelqu'un",
    prouver: "Prouver quelque chose",
    fuir: "S'échapper",
    trouver: "Trouver la vérité",
    gagner: "Relever un défi",
    proteger: "Protéger un secret",
    changer: "Changer sa vie",
    venger: "Réparer une injustice",
  };
  const OBSTACLE_LABELS: Record<string, string> = {
    rival: "Un rival puissant",
    mensonge: "Un mensonge",
    temps: "Le temps qui presse",
    trahison: "Une trahison",
    peur: "Sa propre peur",
    regles: "Les règles",
    solitude: "La solitude",
    secret: "Un secret du passé",
  };
  ...
  return (
    ...
    >
      ...
      <p className="text-xs text-bw-muted text-center leading-relaxed">
        Complète la structure pour raconter l&apos;histoire de{" "}
        <strong className="text-bw-text">{perso?.prenom || "ton personnage"}</strong>.
      </p>
      <div className="w-full space-y-3">
        <div className="p-3 rounded-xl bg-bw-elevated border border-white/[0.06]">
          <p className="text-xs text-bw-muted mb-1">
            C&apos;est l&apos;histoire de <strong className="text-bw-amber">{perso?.prenom || "..."}</strong>
            {perso?.trait ? `, ${perso.trait},` : ""} qui veut{" "}
            <strong className="text-bw-amber">{objectifLabel}</strong>.
          </p>
        </div>
        <div className="p-3 rounded-xl bg-bw-elevated border border-white/[0.06]">
          <p className="text-xs text-bw-muted mb-1">
            Mais <strong className="text-bw-danger">{obstacleLabel}</strong>.
          </p>
        </div>
      ...
      <div className="flex items-center gap-2 w-full">
        {module10.helpEnabled &&
          ["example", "starter", "reformulate"].map((type) => (
            <button
              key={type}
              onClick={() => handleHelp(type)}
              disabled={helpLoading || helpCount >= 3}
        ...
        <AnimatePresence>
          {saved && (
            <motion.span
              ...
              animate={{ opacity: 1, x: 0 }}
              exit={{ opacity: 0 }}
              className="text-xs text-bw-green ml-auto"
            >
              Brouillon sauvé ✓
            </motion.span>
          )}
        ...
        <span
          className={`text-xs ${saved ? "" : "ml-auto"} ${pitchText.trim().length < MIN_PITCH_LENGTH ? "text-bw-amber" : "text-bw-muted"}`}
        >
      ...
      <button
        ...
        disabled={submitting || pitchText.trim().length < MIN_PITCH_LENGTH}
      ...
      >
        {submitting
          ? "Envoi..."
          : pitchText.trim().length < MIN_PITCH_LENGTH
            ? `Encore ${MIN_PITCH_LENGTH - pitchText.trim().length} caractères...`
            : "Valider mon pitch"}
      </button>
    </motion.div>
  );
