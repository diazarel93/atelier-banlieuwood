    ...
    <div className={cn("grid grid-cols-2 sm:grid-cols-4 gap-4", className)}>
      {AXES.map((axis) => (
        ...
        >
          ...
          <StatRing value={scores[axis.key]} label="" color={axis.color} size={72} strokeWidth={5} />
          <span className="text-body-xs font-semibold uppercase tracking-wider" style={{ color: axis.color }}>
            {axis.label}
          </span>
        </div>
      ))}
