...
export function RemoteControlView({
  ...
  const actions = useMemo((): RemoteAction[] => {
    switch (status) {
      case "waiting":
        return [
          { icon: "📺", label: "Projeter", action: () => { updateSession.mutate({ broadcast_message: "__SCREEN_MODE:default", broadcast_at: new Date().toISOString() }); toast.success("Ecran : Question"); } },
          { icon: "▶️", label: "Ouvrir", action: () => { updateSession.mutate({ status: "responding" }); }, color: "#22C55E" },
          { icon: "⏱️", label: "Timer", action: () => { updateSession.mutate({ timer_ends_at: new Date(Date.now() + 60000).toISOString() }); toast.success("Timer 60s"); } },
      ...
      case "voting":
        return [
          ...
          { icon: "📊", label: "Resultats", action: () => updateSession.mutate({ status: "reviewing", timer_ends_at: null }), color: "#8B5CF6" },
          { icon: "⏱️", label: "Timer", action: () => { updateSession.mutate({ timer_ends_at: new Date(Date.now() + 30000).toISOString() }); toast.success("Timer 30s"); } },
          { icon: "⏸️", label: "Pause", action: () => updateSession.mutate({ status: "paused" }) },
        ];
      case "reviewing":
        return [
          ...
          { icon: "📊", label: "Comparer", action: () => { updateSession.mutate({ broadcast_message: "__SCREEN_MODE:responses", broadcast_at: new Date().toISOString() }); toast.success("Ecran : Reponses"); } },
          { icon: "⏸️", label: "Pause", action: () => updateSession.mutate({ status: "paused" }) },
        ];
      case "paused":
        return [
          { icon: "▶️", label: "Reprendre", action: () => updateSession.mutate({ status: "waiting" }), color: "#22C55E" },
          { icon: "📢", label: "Message", action: onOpenBroadcast },
          { icon: "⬛", label: "Noir", action: () => { updateSession.mutate({ broadcast_message: "__SCREEN_MODE:blank", broadcast_at: new Date().toISOString() }); } },
          { icon: "📺", label: "Question", action: () => { updateSession.mutate({ broadcast_message: "__SCREEN_MODE:default", broadcast_at: new Date().toISOString() }); } },
        ];
      default:
        return [
          { icon: "📊", label: "Resultats", action: onNextAction, color: "#8B5CF6" },
          { icon: "📢", label: "Message", action: onOpenBroadcast },
          { icon: "📺", label: "Ecran", action: () => { updateSession.mutate({ broadcast_message: "__SCREEN_MODE:default", broadcast_at: new Date().toISOString() }); } },
          { icon: "⏸️", label: "Pause", action: () => updateSession.mutate({ status: "paused" }) },
        ];
    }
