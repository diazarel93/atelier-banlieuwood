...
function SessionRow({ session }: { session: SessionSummary }) {
  ...
  return (
    <div className="relative flex items-center gap-3 py-3 pl-4 rounded-xl hover:bg-[var(--color-bw-violet)]/[0.04] transition-colors duration-100">
      ...
      {/* Time */}
      <span className="text-xs font-medium text-bw-muted tabular-nums w-10 shrink-0">{time}</span>

      {/* Info */}
      <div className="flex-1 min-w-0">
        <p className="text-heading-xs text-bw-heading truncate">{session.title}</p>
...
function EmptyToday() {
  return (
    <div className="flex flex-col items-center text-center py-4">
      ...
      <p className="text-body-sm text-bw-muted">Journée libre — aucune séance prévue</p>
    </div>
  );
}

export function TodaySessions({ todaySessions, tomorrowSessions, className }: TodaySessionsProps) {
  return (
    <div className={cn("flex flex-col gap-4", className)}>
      ...
      {tomorrowSessions.length > 0 && (
        <GlassCardV2 className="p-5">
          <h3 className="label-caps mb-3">Demain</h3>
          <div className="flex flex-col gap-1">
