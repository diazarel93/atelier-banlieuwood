    ...
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <span className="hero-number-sm text-bw-heading" style={{ fontSize: 24 }}>
            {voteData.totalVotes}
          </span>
        ...
        return (
          <motion.div
            ...
            className={`rounded-xl p-4 border transition-all ${
              i === 0 ? "border-bw-primary/35 bg-bw-primary/[0.06]" : "border-black/[0.06] bg-bw-surface"
            }`}
            style={
              i === 0
                ? { boxShadow: "0 0 20px rgba(255,107,53,0.12), 0 2px 8px rgba(0,0,0,0.2)" }
          ...
          >
            <div className="flex justify-between items-start mb-2">
              ...
              <span
                ...
                style={{ color: i === 0 ? "var(--color-bw-primary)" : "var(--color-bw-muted)" }}
              >
                {pct}%
              </span>
            </div>
            ...
            {vr.voters.length > 0 && (
              <div className="flex gap-1 flex-wrap">
                {vr.voters.map((v, j) => (
                  <span
                    key={j}
                    className="text-xs bg-black/[0.04] px-2 py-0.5 rounded-full text-bw-text border border-black/[0.06]"
                  >
                    {v.avatar} {v.display_name}
                    {v.team_name && (
                      <span className="ml-1 text-xs font-medium" style={{ color: v.team_color }}>
                        {v.team_name}
                      </span>
                    )}
                  </span>
