            ...
            <div className="glass-card !border-bw-pink/20 p-4 space-y-3">
              <div className="flex items-center gap-2">
                <div
                  ...
                  style={{ background: "#EC4899", boxShadow: "0 0 8px rgba(236,72,153,0.5)" }}
                />
              ...
              <p className="text-xs text-bw-muted leading-relaxed">
                Les élèves choisissent <span className="text-bw-heading font-medium">3 contenus minimum</span> parmi ces
                20 oeuvres, puis sélectionnent{" "}
                <span className="text-bw-heading font-medium">celui qui les touche le plus</span>.
              </p>
            </div>
          )}
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs px-2 py-0.5 rounded-full bg-bw-pink/10 text-bw-pink font-bold tabular-nums">
            ...
            <div className="bg-bw-surface rounded-xl border border-black/[0.06] p-6 text-center space-y-3">
              ...
              <div>
                <p className="text-lg font-bold tabular-nums text-bw-teal">
                  {module5Data?.submittedCount || 0}/{activeStudentCount}
                </p>
        ...
        <>
          {!isPreviewing && (
            <div className="glass-card !border-bw-pink/20 p-4 space-y-3">
              ...
              <p className="text-xs text-bw-muted leading-relaxed">
                Chaque élève construit une scène autour de{" "}
                <span className="text-bw-heading font-medium">l&apos;émotion qu&apos;il a choisie</span>. Il rédige{" "}
                <span className="text-bw-heading font-medium">intention + obstacle + changement</span>, puis choisit des
              ...
              <div className="flex gap-3 text-xs">
                <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-xl bg-bw-bg border border-black/[0.06]">
                  <span>📦</span>
                  <span className="text-bw-text">{MAX_SLOTS} emplacements</span>
                </div>
                <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-xl bg-bw-bg border border-black/[0.06]">
                  <span>🪙</span>
                  <span className="text-bw-text">{MAX_TOKENS} jetons max</span>
                </div>
              </div>
            </div>
          )}
            ...
            <div className="space-y-2">
              {scenesData.scenes
                .filter(
                  (sc) =>
                    !cardSearch ||
                    (sc.students?.display_name || "").toLowerCase().includes(cardSearch.toLowerCase()) ||
                    sc.intention?.toLowerCase().includes(cardSearch.toLowerCase()) ||
                    sc.obstacle?.toLowerCase().includes(cardSearch.toLowerCase()),
                )
                .map((sc, i) => {
                    ...
                    >
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm font-medium text-bw-heading">
                          {sc.students?.display_name || "Élève"}
                        </span>
                        <span
                      ...
                      <div className="grid grid-cols-2 gap-1 text-xs text-bw-muted">
                        <div>
                          <span className="text-bw-muted">Intention:</span> {sc.intention}
                        </div>
                        <div>
                          <span className="text-bw-muted">Obstacle:</span> {sc.obstacle}
                        </div>
                      </div>
                      <div className="flex items-center gap-1 flex-wrap">
                        ...
                        <span className="ml-auto text-xs text-bw-muted tabular-nums">
                          {sc.tokens_used}🪙 {sc.slots_used}/5📦
                        </span>
                      </div>
          ...
          {(!scenesData || scenesData.scenes.length === 0) && sessionStatus === "responding" && (
            <div className="bg-bw-surface rounded-xl border border-black/[0.06] p-6 text-center space-y-3">
              ...
              <div>
                <p className="text-lg font-bold tabular-nums text-bw-teal">
                  {scenesData?.count || 0}/{activeStudentCount}
                </p>
        ...
        <>
          {!isPreviewing && (
            <div className="glass-card !border-bw-pink/20 p-4 space-y-3">
              ...
              <p className="text-xs text-bw-muted leading-relaxed">
                Sélectionnez <span className="text-bw-heading font-medium">2 scènes</span> à projeter côte-à-côte. La
                classe compare les choix narratifs et les éléments de mise en scène.
              </p>
            </div>
          )}
            ...
            <div className="space-y-3">
              <div className="bg-bw-pink/10 rounded-xl p-4 border border-bw-pink/20 space-y-2">
                <div className="flex items-center justify-between">
                  <span className="text-sm text-bw-pink font-medium">
                    {scenesData.count} scène{scenesData.count > 1 ? "s" : ""} disponible
                    {scenesData.count > 1 ? "s" : ""}
                  </span>
                  <span className="text-xs text-bw-muted">
                    {selectedSceneIds.length}/2 sélectionnée{selectedSceneIds.length > 1 ? "s" : ""}
                  </span>
                </div>
                {selectedSceneIds.length === 2 && (
                  <button
                    onClick={() =>
                      selectComparison.mutate({ sceneAId: selectedSceneIds[0], sceneBId: selectedSceneIds[1] })
                    }
                    ...
                    return (
                      <motion.div
                        ...
                        className={`bg-bw-surface rounded-xl p-3 border-2 cursor-pointer transition-all space-y-2 ${
                          isSelected
                            ? "border-bw-pink/60 bg-bw-pink/5"
                      ...
                      >
                        ...
                        <div className="text-xs text-bw-muted pl-7 space-y-0.5">
                          ...
                          <div>
                            <span className="text-bw-muted">Changement:</span> {sc.changement}
                          </div>
                        </div>
