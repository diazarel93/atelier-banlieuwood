...
export function SessionCreateWizard() {
  ...
  // Validation (#20)
  const titleError =
    data.title.trim().length > 0 && data.title.trim().length > 100 ? "100 caractères maximum" : undefined;
  const classLabelError = data.classLabel.length > 50 ? "50 caractères maximum" : undefined;
  const thematiqueError = data.thematique.length > 200 ? "200 caractères maximum" : undefined;
  ...
  const canProceed = step === 0 ? data.title.trim().length > 0 && !hasErrors : !hasErrors;

  async function handleCreate() {
    setSaving(true);
    try {
      const count = data.repeat && data.scheduledAt ? data.repeatCount : 1;
      const dayIncrement = data.repeatFrequency === "biweekly" ? 14 : 7;
      ...
      for (let i = 0; i < count; i++) {
        ...
        const sessionTitle = count > 1 ? `${data.title.trim()} (${i + 1}/${count})` : data.title.trim();
          ...
          <div key={label} className="flex items-center gap-2">
            <div
              className={cn(
                ...
                i <= step ? "bg-bw-primary text-white" : "bg-[var(--color-bw-surface-dim)] text-bw-muted",
              )}
            >
              {i + 1}
            </div>
            <span className={cn("text-xs font-medium", i <= step ? "text-bw-heading" : "text-bw-muted")}>{label}</span>
            {i < 2 && (
              <div
                className={cn(
      ...
      <GlassCardV2 className="p-6 overflow-hidden">
        <AnimatePresence mode="wait">
          ...
          {step === 0 && (
            ...
            >
              <h2 className="text-lg font-bold text-bw-heading">Créer une séance</h2>
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-medium text-bw-muted">Formule</label>
                <div className="flex flex-col gap-1.5">
              ...
              {data.scheduledAt && (
                <div className="flex flex-col gap-3">
                  <label className="flex items-center gap-2.5 cursor-pointer">
                    ...
                    <span className="text-xs font-medium text-bw-heading">Repeter</span>
                  </label>

                  {data.repeat && (
                    <div className="flex flex-col gap-2 pl-0.5">
                      <div className="flex flex-col gap-1.5">
                        <label className="text-xs font-medium text-bw-muted">Frequence</label>
                        <div className="flex gap-2">
                          {(
                            [
                            ...
                            <button
                              key={opt.value}
                              type="button"
                              onClick={() => update({ repeatFrequency: opt.value })}
                              className={cn(
                      ...
                      <div className="flex flex-col gap-1.5">
                        <label htmlFor="wizard-repeat-count" className="text-xs font-medium text-bw-muted">
                          Nombre de seances
                        </label>
                        <select
                          id="wizard-repeat-count"
                          value={data.repeatCount}
                          onChange={(e) => update({ repeatCount: parseInt(e.target.value) })}
          ...
          {step === 1 && (
            <motion.div
              ...
              className="flex flex-col gap-4"
            >
              <h2 className="text-lg font-bold text-bw-heading">Options</h2>
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-medium text-bw-muted">Template de genre</label>
          ...
          {step === 2 && (
            <motion.div
              ...
              className="flex flex-col gap-4"
            >
              <h2 className="text-lg font-bold text-bw-heading">Récapitulatif</h2>
              <div className="flex flex-col gap-2 text-sm">
                <Row label="Titre" value={data.title} />
                <Row label="Classe" value={data.classLabel || "—"} />
                <Row label="Niveau" value={LEVELS.find((l) => l.value === data.level)?.label || data.level} />
                <Row
                  label="Date"
                  value={data.scheduledAt ? new Date(data.scheduledAt).toLocaleString("fr-FR") : "Non planifiée"}
                />
                <Row label="Template" value={TEMPLATES.find((t) => t.value === data.template)?.label || "Aucun"} />
