...
export function Module5EmotionDistribution({ emotionDistribution }: Module5EmotionDistributionProps) {
  ...
  return (
    <div className="bg-bw-surface rounded-xl p-4 border border-black/[0.04] space-y-3">
      <span className="text-xs font-semibold uppercase tracking-wider text-bw-muted">
        Distribution des émotions choisies
      </span>
          ...
          return (
            ...
            >
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium" style={{ color: emo?.color || "#EC4899" }}>
                  {emo?.label || key}
                </span>
                <span className="text-xs tabular-nums" style={{ color: emo?.color || "#EC4899" }}>
                  {pct}% ({count})
                </span>
              </div>
              <div className="h-2 bg-bw-bg rounded-full overflow-hidden">
                <motion.div
                  ...
                  style={{ backgroundColor: emo?.color || "#EC4899" }}
                />
              </div>
            </motion.div>
          );
