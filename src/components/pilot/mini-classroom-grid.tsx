...
export function MiniClassroomGrid({ studentStates, onStudentClick }: MiniClassroomGridProps) {
  ...
  const attentionStudents = studentStates.filter((s) => s.state === "stuck" || s.hand_raised_at);
        ...
        {studentStates.map((s) => {
          ...
          return (
            ...
            >
              ...
              {/* Hand raised badge */}
              {s.hand_raised_at && (
                <span className="absolute -top-1 -right-1 text-[10px] animate-bounce drop-shadow-sm">✋</span>
              )}
            </button>
          );
        })}
      ...
      <div className="flex items-center justify-center gap-3 mt-2.5">
        {LEGEND.map((item) => (
          <div key={item.state} className="flex items-center gap-1">
            <span className="w-2 h-2 rounded-full flex-shrink-0" style={{ background: STATE_DOT[item.state] }} />
